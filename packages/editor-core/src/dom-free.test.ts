...
/**
 * DOM-free invariant. `@vision-control/editor-core` is `platform:isomorphic`
 * and consumed by Node packages (daemon) and browser packages (overlay-ui,
 * extension). It MUST NOT reference any browser-only global. See the
 * element-identity sibling test for the full rationale.
 */
...
describe("editor-core is DOM-free", () => {
  ...
  it("imports cleanly under a Node environment (no ReferenceError at module load)", async () => {
    ...
    expect(typeof mod.createMultiSelectGroup).toBe("function");
    expect(typeof mod.evaluateGroupConstraints).toBe("function");
  });
});
