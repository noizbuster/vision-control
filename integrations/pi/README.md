...
(a URL endpoint). Both transports are supported. The exact name and shape of
Pi's server-entry setting evolves independently of this repo, so map the fields
below onto Pi's current MCP client settings. The transport contract is what
matters and is stable.

### stdio (recommended)

Pi spawns the server as a child process and talks JSON-RPC over stdin/stdout.
This is the simplest path and needs no HTTP endpoint.

Transport fields:

- transport: `stdio`
- command: `["pnpm", "exec", "vision-control-mcp"]`
- environment: `{ "VC_DAEMON_URL": "http://127.0.0.1:4321" }`
...
[examples/pi.stdio.json](./examples/pi.stdio.json).
...
[examples/pi.http.json](./examples/pi.http.json).

## Smoke test the connection

After wiring the config, ask Pi to call the session tool to confirm the server
is reachable:
...
The `@vision-control/pi` package exports typed builders so a script or a doc
generator can emit the server entry without hand-editing JSON:
...
`buildPiConfig(entry)` wraps one under the `vision-control` label. The builders
take a token only from the caller; they never embed a secret.

## Troubleshooting

**The `vision-control` server is not listed by Pi.** Confirm the server builds
...
Then check that Pi's MCP server entry points at the right command or URL.
...
`fetch` without that header gets 406. Pi's MCP client sets it; a custom client
must too.
...
```bash
pnpm nx run pi:build        # tsc -p tsconfig.build.json -> dist/
pnpm nx run pi:typecheck    # tsc --noEmit -p tsconfig.json
pnpm nx run pi:test         # vitest run
```
