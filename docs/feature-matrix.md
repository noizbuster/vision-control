...
| Auto Layout (Hug / Fill / Fixed) |  -  | done  -  wired + unit-tested; browser e2e pending | Panel + on-page Layout-mode overlay (`apps/extension/src/overlay/auto-layout-overlay.ts`, shared builders in `auto-layout-operations.ts`). `packages/layout-engine/src/auto-layout/`. Feature-specific panel e2e pending. |
