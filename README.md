...
4. Open DevTools on any http(s) page (any hostname, including local dev
   servers). The **Vision Control** panel appears.

The built manifest covers all `http://*/*` and `https://*/*` pages.
`debugger` is an `optional_permissions` entry and is never required. The MCP
bridge still binds loopback-only. See
[apps/extension/README.md](./apps/extension/README.md) for the permissions
rationale.
