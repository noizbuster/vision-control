...
/**
 * DOM-free invariant. `@vision-control/layout-engine` is `platform:isomorphic`
 * and consumed by Node packages. It MUST NOT reference any browser-only global.
 */
...
describe("layout-engine is DOM-free", () => {
  ...
  it("imports cleanly under a Node environment (no ReferenceError at module load)", async () => {
    ...
    expect(typeof mod.classifyLayoutRole).toBe("function");
    expect(typeof mod.computeInsertionIndex).toBe("function");
  });
});
